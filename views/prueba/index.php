...
$template = '';
if (\Yii::$app->user->can('/prueba/view')) {
    $template .= '{view} ';
}
if (\Yii::$app->user->can('/prueba/update')) {
    $template .= '{update} ';
}
if (\Yii::$app->user->can('/prueba/delete')) {
    $template .= '{delete} ';
}
if (\Yii::$app->user->can('/prueba/*') || \Yii::$app->user->can('/*')) {
    $template = '{view}  {update}  {delete}';
}
?>
<div class="prueba-index box box-primary">
    <div class="box-header with-border">
    <?php  if (\Yii::$app->user->can('/prueba/create') || \Yii::$app->user->can('/*')) :  ?> 
        <?= Html::a('<i class="flaticon-add" style="font-size: 20px"></i> '.'Crear Prueba', ['create'], ['class' => 'btn btn-primary']) ?>
    <?php  endif;  ?> 
    </div>
