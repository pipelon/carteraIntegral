...
?>
<div class="etapas-procesales-cuaderno-ppal-view box box-primary">
    <div class="box-header">
        <?php  if (\Yii::$app->user->can('/etapas-procesales-cuaderno-ppal/index') || \Yii::$app->user->can('/*')) :  ?>        
            <?= Html::a('<i class="flaticon-up-arrow-1" style="font-size: 15px"></i> '.'Volver', ['index'], ['class' => 'btn btn-default']) ?>
        <?php  endif;  ?> 
        <?php  if (\Yii::$app->user->can('/etapas-procesales-cuaderno-ppal/update') || \Yii::$app->user->can('/*')) :  ?>        
            <?= Html::a('<i class="flaticon-edit-1" style="font-size: 15px"></i> '.'Actualizar', ['update', 'id' => $model->id], ['class' => 'btn btn-primary']) ?>
        <?php  endif;  ?> 
        <?php  if (\Yii::$app->user->can('/etapas-procesales-cuaderno-ppal/delete') || \Yii::$app->user->can('/*')) :  ?>        
